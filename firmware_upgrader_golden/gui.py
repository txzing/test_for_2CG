...
#***********************************************
    ...
    def setupUi(self):
        ...
        self.golden_button.setDisabled(True)
    ...
    def ConnectSignalSlot(self):
        ...
        self.golden_button.clicked.connect(self.golden_file_handler)
        self.ReadButton.clicked.connect(self.read_info_handler)
    ...
    def DisconnectSignalSlot(self):     
        ...
        self.golden_button.clicked.disconnect(self.golden_file_handler)
        self.ReadButton.clicked.disconnect(self.read_info_handler)
    ...
    def closeEvent(self, event):
        ...
        if(self.connection_status == True):
            ...
            try:
                ...
                self.ut.close()
                del self.ut
    def read_info_handler(self):
        if(self.connection_status == True):
            status=self.vm1.read_var(WRITE_ADDR_IDX)
            print(status[1])
            if status[0]:
                print("ok, 0x%x"%status[1])
                self.write_lable.setText("0x"+("%x"%status[1]).upper())
            else:
                QMessageBox.information(self,"Error!", "communication status err") 
                print("ng")
            
            status=self.vm1.read_var(FLASH_INFO_IDX)
            print(status[1])
            if status[0]:
                print("ok," + status[1].decode('utf-8'))
                self.make_lable.setText(status[1].decode('utf-8'))
            else:
                QMessageBox.information(self,"Error!", "communication status err") 
                print("ng")

            status=self.vm1.read_var(FLASH_SIZE_IDX)
            print(status[1])
            if status[0]:
                print("ok, 0x%x"%status[1])
                self.size_lable.setText(str(status[1])+" KB")
            else:
                QMessageBox.information(self,"Error!", "communication status err") 
                print("ng")
        pass
    ...
    def file_handle(self,text):
        ...
        if self.comboBox.currentText() == 'UDP':
            self.udpThread = udp_update.UDP_sent_thread(self.textEdit_upgradeFile.text(),self.ipaddr_lineEdit.text(),self.port_set.text(),text)
            self.udpThread.finishSignal.connect(self.RecvSendTreadSinalHandler)
            self.udpThread.start()
        else:
            self.tcpThread = tcp_update.TCP_sent_thread(self.textEdit_upgradeFile.text(),self.ipaddr_lineEdit.text(),self.port_set.text(),text)
    ...
    def golden_file_handler(self):
        self.file_handle('golden')
    ...
    def ipaddr_connect_btn_handler(self):
        ...
        if status is False:
            ...
            if(self.connection_status == True):
                ...
                self.golden_button.setDisabled(False)
    ...
    def clear_display(self):
        ...
        self.write_lable.setText("?")
        self.make_lable.setText("?")
        self.size_lable.setText("?")   
        self.upgrade_file_button.setDisabled(True)
        self.upgrade_button.setDisabled(True)
        self.ReadButton.setDisabled(True)
    ...
    def check_connection(self, ip_addr):
        ...
        if(status is True):
            print("%s ping ok"%ip_addr)
        else:
            print("%s ping faild"%ip_addr)
