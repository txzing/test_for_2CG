...
#***********************************************
    ...
    def upgrade_file_handler(self):
        ...
        if self.comboBox.currentText() == 'UDP':
            self.udpThread = udp_update.UDP_sent_thread(self.textEdit_upgradeFile.text(),self.ipaddr_lineEdit.text(),self.port_set.text())
            self.udpThread.finishSignal.connect(self.RecvSendTreadSinalHandler)
            self.udpThread.start()
        else:
            self.tcpThread = tcp_update.TCP_sent_thread(self.textEdit_upgradeFile.text(),self.ipaddr_lineEdit.text(),self.port_set.text())
