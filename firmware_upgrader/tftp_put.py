...
class tftp_md5_put_tread(QtCore.QThread):  # The timer class is derived from the class threading.Thread
    ...
    def __init__(self, uploadFileName, server_ip, sever_port,parent=None):
        super(tftp_md5_put_tread, self).__init__(parent)
        self.uploadFileName = uploadFileName
        self.server_ip = server_ip
        self.sever_port = int(sever_port)
    ...
    def run(self):  # Overwrite run() method, put what you want the thread do here
        ...
        s.sendto(sendDataFirst, (self.server_ip, self.sever_port))  # 第一次发送, 连接tftp服务器
        
        s.sendto(md5string.encode('utf-8'), (self.server_ip, self.sever_port))  # 第一次发送, 连接tftp服务器
        ...
        while not self.thread_stop:
            ...
            if pre_percent == 0 and send_len == 1024:
                ...
                self.finishSignal.emit(["Transmission complete: 0%"]) 
            # 　打包
            # sendData = struct.pack('!HH', 3, fileNum) + readFileData

            # 发送数据到tftp服务器
            # s.sendto(sendData, serverInfo)  # 第二次发给服务器的随机端口
            s.sendto(readFileData, serverInfo)
            time.sleep(0.001)
            ...
            if pre_percent != process_percent:
                ...
                if process_percent in data:
                    self.finishSignal.emit(["Transmission complete: " + str(process_percent) + "%"]) 
            # time.sleep(0.001)
            ...
            if len(readFileData) < 1024:
                ...
                # s.close()
                time.sleep(0.01)
                s.sendto(b"update", (self.server_ip, self.sever_port))
                break
