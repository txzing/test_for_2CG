...
class memory_ops():
    ...
    def read_reg(self, reg_addr):
        ...
        status = self.ut.recieved_even.wait(0.2)
        result=[]
    ...
    def read_bulk(self, reg_addr, cnt):
        if (cnt>358):
            raise Exception("cnt should not bigger than 358")
    ...
    def write_bulk(self, reg_addr, send_data):
        if len(send_data) > 358 :
            raise Exception("len of send_data should not bigger than 358")
