		...
		case XFSBL_STAGE1:
			{
				...
				//while(!XUartPs_IsTransmitEmpty(STDOUT_BASEADDRESS));
				while(Timeout<5000)
				{
				...
				Status1 = XFsbl_In32(0x00FFD80064);
				Xil_Out32(0x00FFD80064, 0x00000000);
				XFsbl_Printf(DEBUG_GENERAL,"PERS_GLOB_GEN_STORAGE5 Status: 0x%08lx\r\n", Status1);
				Status2 = XFsbl_In32(CRL_APB_RESET_REASON);
				XFsbl_Printf(DEBUG_GENERAL,"\n\rResetReason = 0x%.8X\n\r", Status2);
				if(Status2 & 0x40)
					XFsbl_Printf(DEBUG_GENERAL,"6 Software Debugger Reset\n\r");
				if(Status2 & 0x20)
					XFsbl_Printf(DEBUG_GENERAL,"5 Software System Reset\n\r");
				if(Status2 & 0x10)
					XFsbl_Printf(DEBUG_GENERAL,"4 External System Reset\n\r");
				if(Status2 & 0x08)
					XFsbl_Printf(DEBUG_GENERAL,"3 PS-only Reset\n\r");
				if(Status2 & 0x04)
					XFsbl_Printf(DEBUG_GENERAL,"2 Internal System Reset\n\r");
				if(Status2 & 0x02)
					XFsbl_Printf(DEBUG_GENERAL,"1 A system error triggered a POR reset\n\r");
				if(Status2 & 0x01)
					XFsbl_Printf(DEBUG_GENERAL,"0 the PS_POR_B reset signal pin was asserted\n\r");
				XFsbl_Printf(DEBUG_GENERAL,"\n\r");

				if ((Status==0) && ( (Status1 & 0x80000000) || ((Status2 & 0x20)==0) ) )
				{
					XFsbl_Out32(CSU_CSU_MULTI_BOOT, 0x60);
				}
				else
				{
					XFsbl_Out32(CSU_CSU_MULTI_BOOT, 0x0);
				}
			}break;
...
void XFsbl_PrintFsblBanner(void )
{
	...
	XFsbl_Printf(DEBUG_PRINT_ALWAYS,
                 "Xilinx Zynq MP First Stage Boot Loader. Press Blank to enter golden \n\r");
	XFsbl_Printf(DEBUG_PRINT_ALWAYS,
                 "Release %d.%d   %s  -  %s\r\n",
